...
pub type DtmMap<'a> = HashMap<Signature, (Mmap, &'a [u16])>;
...
impl CPos {
    ...
    ///
    /// By `CPos::unapply_general`ing the generated moves to the `CPos` one obtains possible positions (not necessarily
    ...
    /// For example:
    /// ```
    /// cpos.unapply_general(Move:new(WHITE, PAWN, QUEEN, A7, B8), ROOK)
    /// ```
    ...
    ///             cpos.unapply_general(mv, piece); // where piece is not EMPTY or KING
    ///                                              // and PAWN only if no promotion
    ///         }
    ///         else { cpos.unapply_general(mv, EMPTY) }     // or any piece except KING and PAWN on promotion rank
    ...
    /// Returns a simpler reverse iterator that does not generate capturing PAWN moves or promotions.
    /// You can safely [CPos::unapply_nc] such moves. However, the result must still be checked for validity.
    ///
    /// See also [CPos::predecessors]
    pub fn simple_reverse_move_iterator(&self, player: Player) -> CPosSimpleReverseMoveIterator {
    ...
    pub fn unapply_general(&self, mv: Move, uncapture: Piece) -> CPos {
        let mut new = *self;
    ...
    /// Unapply a non-capturing move that is also not a PAWN un-promotion.
    ///
    /// You should use this **only** with moves generated by the [CPosSimpleReverseMoveGenerator] as it does not do any checking for speed.
    pub fn unapply_nc(&self, mv: Move) -> CPos {
        let mut new = *self;
    ...
    pub fn dtm_mmap<'a>(canonsig: Signature, hash: &'a mut DtmMap) -> Result<(), String> {
        if !hash.contains_key(&canonsig) {
            let path = mk_egtb_path(canonsig, "dtm");
    ...
    /// Compute the distance to mate for player
    /// - if this is a loosing position, the DTM is the maximum of the DTM's of the successors + 1 (or 0 for mate positions)
    /// - if this is a winning position, the DTM is looked up in the DTM table
    pub fn find_dtm(
        &self,
        player: Player,
        e_hash: &mut EgtbMap,
        ...
        let canonic = self.clear_trans().find(e_hash)?;
        let canonsig = canonic.signature();
        let cplayer = if canonic.canonic_has_bw_switched() { player.opponent() } else { player };
        if canonic.state(cplayer) == WINS {
            CPos::dtm_mmap(canonsig, d_hash)?;
            let dtms = d_hash.get(&canonsig).unwrap().1;
            let n = canonic.find_canonic_mpos_index(canonsig, cplayer, m_hash)?;
            Ok(dtms[n])
        } else if canonic.state(cplayer) == LOST {
            canonic
                .successors(cplayer)
            ...
            if *self == canonic {
                ...
                Err(format!("no DTM for canonic {:?}  from {:?}", canonic, self))
            }
        }
    }
